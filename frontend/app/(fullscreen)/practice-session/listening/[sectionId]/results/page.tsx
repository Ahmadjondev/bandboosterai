...
/**
 * Unified Listening Results Page
 * Supports both book sections (numeric IDs) and section practices (UUIDs)
 * URL: /practice-session/listening/[sectionId]/results
 */
...
export default function ListeningResultsPage() {
  ...
  const getEncouragementMessage = (score: number) => {
    if (score >= 8.5) return 'Outstanding! You have excellent listening skills! 🌟';
    if (score >= 8.0) return 'Excellent work! You have very good listening comprehension! 🎉';
    if (score >= 7.0) return 'Great job! You have good listening skills! 👏';
    if (score >= 6.0) return 'Good effort! You have competent listening skills. Keep practicing! 💪';
    if (score >= 5.0) return 'Keep going! More listening practice will help! 📚';
    return 'Don\'t give up! Regular practice will improve your listening skills! 🚀';
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950">
      ...
      <div className="relative overflow-hidden bg-gradient-to-br from-blue-500 via-blue-600 to-indigo-700 dark:from-blue-600 dark:via-blue-700 dark:to-indigo-800">
        {/* Decorative circles */}
        ...
        <div className="relative max-w-5xl mx-auto px-4 pt-8 pb-16 sm:px-6 lg:px-8">
          ...
          <div className="text-center">
            <div className="inline-flex items-center gap-2 px-3 py-1 bg-white/20 backdrop-blur-sm rounded-full text-white/90 text-sm font-medium mb-4">
              <span>🎧</span>
              <span>Listening Practice Complete</span>
            </div>
            <h1 className="text-2xl sm:text-3xl font-bold text-white mb-1">
              {result.section.title}
            </h1>
            <p className="text-blue-100">{result.book.title}</p>
          </div>
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
        ...
        <div className="flex flex-col sm:flex-row gap-3 pt-4 pb-8">
          <button
            onClick={() => router.push(`/practice-session/listening/${sectionIdParam}`)}
            className="flex-1 inline-flex items-center justify-center gap-2 px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-xl transition-colors shadow-lg shadow-blue-600/20 hover:shadow-blue-600/30"
          >
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
