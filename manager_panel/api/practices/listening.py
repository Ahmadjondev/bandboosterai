"""
Listening Section Practices API
CRUD operations for listening practices with bulk add support.
"""
...
def get_listening_content_info(practice, detailed=False):
    """Get listening-specific content info."""
    if not practice.listening_part:
        return {}
...
def get_listening_practices(request):
    """
    Get list of all listening practices with filtering and pagination.
    ...
    practices = get_base_queryset("LISTENING")
    ...
    for practice in paginated["results"]:
        ...
        data["content"] = get_listening_content_info(practice)
        results.append(data)
...
@api_view(["GET"])
@permission_classes([IsAuthenticated])
def get_listening_practice(request, practice_id):
    """Get a single listening practice by ID with full details."""
    perm_error = check_permission(request)
    if perm_error:
        return perm_error

    practice = get_object_or_404(
        SectionPractice.objects.select_related("listening_part", "created_by"),
        id=practice_id,
        section_type="LISTENING",
    )

    data = serialize_practice(practice, detailed=True)
    data["content"] = get_listening_content_info(practice, detailed=True)
...
@api_view(["POST"])
@permission_classes([IsAuthenticated])
def create_listening_practice(request):
    """
    Create a new listening practice.

    Body (JSON):
        - content_id: ID of the ListeningPart to link (required)
    ...
    try:
        listening_part = ListeningPart.objects.get(id=content_id)
    except ListeningPart.DoesNotExist:
        return Response(
            {"error": f"ListeningPart with ID {content_id} not found"},
            status=status.HTTP_404_NOT_FOUND,
        )

    # Check if already linked
    existing = SectionPractice.objects.filter(
        section_type="LISTENING", listening_part=listening_part
    ).first()
    if existing:
        return Response(
            {
                "error": f"This listening part is already linked to practice '{existing.title}'"
            },
            status=status.HTTP_400_BAD_REQUEST,
        )
    ...
    total_questions = sum(
        th.questions.count() for th in listening_part.test_heads.all()
    )

    # Create practice
    title = (
        request.data.get("title")
        or listening_part.title
        or f"Listening Part {listening_part.part_number}"
    )
    ...
    return Response(
        {
            "success": True,
            "message": "Listening practice created successfully",
...
@api_view(["PUT", "PATCH"])
@permission_classes([IsAuthenticated])
def update_listening_practice(request, practice_id):
    ...
    practice = get_object_or_404(
        SectionPractice, id=practice_id, section_type="LISTENING"
    )
    ...
    return Response(
        {
            "success": True,
            "message": "Listening practice updated successfully",
            "practice": serialize_practice(practice),
        }
    )
...
@api_view(["POST"])
@permission_classes([IsAuthenticated])
def toggle_listening_practice_status(request, practice_id):
    """Toggle the active status of a listening practice."""
    perm_error = check_permission(request)
    if perm_error:
        return perm_error
...
@api_view(["DELETE"])
@permission_classes([IsAuthenticated])
def delete_listening_practice(request, practice_id):
    ...
    return Response(
        {
            "success": True,
            "message": f"Listening practice '{title}' deleted successfully",
        }
    )
...
@api_view(["POST"])
@permission_classes([IsAuthenticated])
def bulk_create_listening_practices(request):
    """
    Create multiple listening practices at once.

    Body (JSON):
        - content_ids: List of ListeningPart IDs to create practices for (required)
    ...
    for content_id in content_ids:
        try:
            ...
            title = (
                listening_part.title or f"Listening Part {listening_part.part_number}"
            )
            ...
            errors.append({"id": content_id, "error": "ListeningPart not found"})
...
@api_view(["GET"])
@permission_classes([IsAuthenticated])
def get_available_listening_content(request):
    """
    Get listening parts that are not yet linked to any practice.
    """
    perm_error = check_permission(request)
    if perm_error:
        return perm_error
    ...
    content = ListeningPart.objects.exclude(id__in=linked_ids).order_by("-created_at")
