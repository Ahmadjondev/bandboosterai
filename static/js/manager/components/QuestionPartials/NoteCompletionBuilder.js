...
const NoteCompletionBuilderComponent = {
    template: `
        <div class="bg-white rounded-lg p-6 shadow-sm">
            <!-- Header -->
            <div class="mb-6 pb-4 border-b-2 border-gray-200">
                <h3 class="text-2xl font-semibold text-gray-800 mb-2">📝 Note Completion Builder</h3>
                <p class="text-gray-500 text-sm">Create structured notes with sections, items, and blanks</p>
            </div>
                ...
                <div class="flex flex-col gap-5">
                    ...
                    <div class="bg-gray-50 rounded-lg p-5">
                        <div class="flex justify-between items-center mb-4">
                            <label class="text-base font-semibold text-gray-700">Sections</label>
                            <button 
                                type="button" 
                        ...
                        <!-- Empty State -->
                        <div v-if="noteStructure.items.length === 0" class="text-center py-10 text-gray-500">
                            <i data-feather="file-text" class="w-12 h-12 mx-auto mb-4 opacity-30"></i>
                            <p class="text-sm">No sections yet. Click "Add Section" to start.</p>
                        </div>

                        <!-- Section List -->
                        <div 
                            v-for="(section, sIndex) in noteStructure.items" 
                            :key="'section-' + sIndex"
                        ...
                        >
                            ...
                            <div class="bg-gray-50 rounded-md p-3">
                                <div class="flex justify-between items-center mb-3">
                                    ...
                                    >
                                        <i data-feather="plus" class="w-3 h-3"></i> Add Item
                                    </button>
                                </div>

                                <div v-if="section.items.length === 0" class="text-center py-5">
                                    <p class="text-xs text-gray-500">No items. Click "Add Item" to start.</p>
                                </div>
                                    ...
                                    <div v-if="typeof item === 'string'" class="flex gap-3 items-start">
                                        ...
                                        <div class="flex-1 flex flex-col gap-2">
                                            <textarea 
                                                ...
                                                placeholder="Item text (use <input> for blanks)"
                                                rows="2"
                                                @input="markAsModified"
                                            ></textarea>
                                            <div class="flex gap-2 flex-wrap">
                                                ...
                                                >
                                                    <i data-feather="list" class="w-3 h-3"></i> Convert to Nested
                                                </button>
                                                <button 
                                                    type="button" 
                                        ...
                                        <div class="flex-1 flex flex-col gap-3">
                                            ...
                                            <div class="bg-gray-100 rounded-md p-3 ml-5">
                                                ...
                                                >
                                                    ...
                                                    <textarea 
                                                        ...
                                                        placeholder="Sub-item text (use <input> for blanks)"
                                                        rows="1"
                                                        @input="markAsModified"
                                                    ></textarea>
                                                    <button 
                                                ...
                                                >
                                                    <i data-feather="plus" class="w-3 h-3"></i> Add Sub-item
                                                </button>
                                            </div>
                                            <div class="flex gap-2">
                                                ...
                                                <button 
                                                    ...
                                                    @click="removeItem(sIndex, iIndex)"
                                                >
                                                    <i data-feather="trash-2" class="w-3 h-3"></i> Delete Nested Item
                                                </button>
                                            </div>
                                        </div>
                                    </div>
                    ...
                    <div class="bg-gray-50 border border-gray-200 rounded-lg p-5">
                        <h5 class="text-base font-semibold text-gray-700 mb-4">Preview</h5>
            ...
            <div v-if="currentStep === 2">
                <div class="flex flex-col gap-5">
                    <div class="flex items-center gap-3 p-3 bg-orange-50 border border-orange-200 rounded-lg text-orange-700">
                        ...
                        <span class="text-sm">{{ generatedQuestions.length }} question(s) generated. Set the correct answers below.</span>
                    </div>
    ...
    computed: {
        canGenerate() {
            if (!this.noteStructure.title.trim()) return false;
            if (this.noteStructure.items.length === 0) return false;
    ...
    mounted() {
        ...
        if (this.noteData) {
            try {
                const parsed = JSON.parse(this.noteData);
                this.noteStructure = parsed;
    ...
    methods: {
        ...
        removeSection(sIndex) {
            if (confirm('Are you sure you want to delete this section?')) {
                this.noteStructure.items.splice(sIndex, 1);
                this.markAsModified();
            }
        },
        ...
        moveSectionUp(sIndex) {
            if (sIndex > 0) {
                const temp = this.noteStructure.items[sIndex];
                this.noteStructure.items.splice(sIndex, 1);
                this.noteStructure.items.splice(sIndex - 1, 0, temp);
                this.markAsModified();
            }
        },

        moveSectionDown(sIndex) {
            if (sIndex < this.noteStructure.items.length - 1) {
                const temp = this.noteStructure.items[sIndex];
                this.noteStructure.items.splice(sIndex, 1);
                this.noteStructure.items.splice(sIndex + 1, 0, temp);
                this.markAsModified();
        ...
        addItem(sIndex) {
            this.noteStructure.items[sIndex].items.push('');
            this.markAsModified();
        },

        removeItem(sIndex, iIndex) {
            this.noteStructure.items[sIndex].items.splice(iIndex, 1);
            this.markAsModified();
        },

        convertToNested(sIndex, iIndex) {
            const currentText = this.noteStructure.items[sIndex].items[iIndex];
            this.noteStructure.items[sIndex].items.splice(iIndex, 1, {
                prefix: '',
                items: [currentText || '']
            });
        ...
        convertToSimple(sIndex, iIndex) {
            const nestedItem = this.noteStructure.items[sIndex].items[iIndex];
            const text = nestedItem.prefix || (nestedItem.items[0] || '');
            this.noteStructure.items[sIndex].items.splice(iIndex, 1, text);
            this.markAsModified();
        },

        addSubItem(sIndex, iIndex) {
            const item = this.noteStructure.items[sIndex].items[iIndex];
        ...
        insertBlankInItem(sIndex, iIndex) {
            const currentText = this.noteStructure.items[sIndex].items[iIndex];
            this.noteStructure.items[sIndex].items[iIndex] = currentText + ' <input> ';
            this.markAsModified();
        },
        ...
        countInputTags() {
            let count = 0;
            const jsonStr = JSON.stringify(this.noteStructure);
        ...
        generateQuestions() {
            if (!this.canGenerate) {
                alert('Please add a title and at least one <input> blank.');
                return;
            }
            ...
            this.noteStructure.items.forEach((section, sIndex) => {
        ...
        renderPreview() {
            if (!this.noteStructure.title && this.noteStructure.items.length === 0) {
                return '<p class="text-gray-500">No content to preview</p>';
            }

            let html = '';

            if (this.noteStructure.title) {
                html += `<h4 class="text-xl font-bold text-gray-800 mb-5">${this.escapeHtml(this.noteStructure.title)}</h4>`;
            }

            this.noteStructure.items.forEach((section, sIndex) => {
                if (section.title) {
                    html += `<h5 class="text-base font-semibold text-gray-700 mt-5 mb-3">${this.escapeHtml(section.title)}</h5>`;
                }
                html += '<ul class="list-disc pl-6 space-y-2">';
                section.items.forEach((item, iIndex) => {
                    if (typeof item === 'string') {
                        const rendered = item.replace(/<input>/gi, '<span class="inline-block border-b-2 border-orange-500 min-w-[80px] px-1 text-orange-600 font-semibold">_____</span>');
                        html += `<li class="leading-relaxed">${rendered}</li>`;
                    } else if (item.items) {
                        html += '<li class="leading-relaxed">';
                        if (item.prefix) {
                            html += `<strong>${this.escapeHtml(item.prefix)}</strong>`;
                        }
                        html += '<ul class="list-circle pl-6 mt-2 space-y-1">';
                        item.items.forEach(subItem => {
                            const rendered = subItem.replace(/<input>/gi, '<span class="inline-block border-b-2 border-orange-500 min-w-[80px] px-1 text-orange-600 font-semibold">_____</span>');
                            html += `<li>${rendered}</li>`;
                        });
                        html += '</ul></li>';
                    }
        ...
        saveQuestions() {
            ...
            this.$emit('update:note-data', JSON.stringify(this.noteStructure, null, 2));
        }
    }
};
...
if (window.app) {
    window.app.component('note-completion-builder', NoteCompletionBuilderComponent);
}

// Export for module usage
if (typeof module !== 'undefined' && module.exports) {
    module.exports = NoteCompletionBuilderComponent;
}

// Expose to window for direct script tag usage
window.NoteCompletionBuilderComponent = NoteCompletionBuilderComponent;
